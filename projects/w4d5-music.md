...
* `#logged_in?` to return a boolean indicating whether someone is
  signed in.
* `#log_in_user!(user)` reset the `user`s session token, cookie, &c.
...
* Put a `Note` form on the show page.
    ...
    * For this form, you can `redirect_to` the track even on
      ...
      form. OTOH, the only real validation is that the content not
      be blank...
...
* Add destroy buttons for notes to the `notes/_note` partial.
    ...
    * Issue a 403 FORBIDDEN status if the user tries to destroy a note
      but they are not allowed to. You can use `render text:` to give
      an bare-bones error message; only malicious users would see this.
